...
{
    internal abstract class ObjectContainerHelperBase : ContainerHelper
    {
        ...
        private bool isPreparingItemFlag = false;
        private PropertyItemCollection propertyItemCollection;
        ...
        {
            get { return propertyItemCollection; }
        }
        ...
        private PropertyItem DefaultProperty
        {
            get
            {
                ...
                if (defaultName != null)
                {
                    defaultProperty = propertyItemCollection.FirstOrDefault((prop) => object.Equals(defaultName, prop.PropertyDescriptor.Name));
                }
        ...
        private void UpdateFilter()
        {
            ...
            this.PropertyItems.FilterPredicate = filterInfo.Predicate ?? PropertyItemCollection.CreateFilter(filterInfo.InputString, this.PropertyItems, this.PropertyContainer);
        }

        private void UpdateCategorization(bool updateSubPropertiesCategorization)
        {
            propertyItemCollection.UpdateCategorization(this.ComputeCategoryGroupDescription(), this.PropertyContainer.IsCategorized, this.PropertyContainer.IsSortedAlphabetically);
            if (updateSubPropertiesCategorization && (propertyItemCollection.Count > 0))
            {
                foreach (PropertyItem propertyItem in propertyItemCollection)
                {
                    if (propertyItem.Properties is PropertyItemCollection subPropertyItemsCollection)
                    {
        ...
        {
            return (this.ComputeCategoryGroupDescription() is PropertyGroupDescription propGroup) ? propGroup.PropertyName : null;
        }

        private void OnChildrenPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (ObjectContainerHelperBase.IsItemOrderingProperty(e.PropertyName) || this.GetCategoryGroupingPropertyName() == e.PropertyName)
            {
                // Refreshing the view while Containers are generated will throw an exception
                if (this.ChildrenItemsControl.ItemContainerGenerator.Status != GeneratorStatus.GeneratingContainers && !isPreparingItemFlag)
                {
        ...
        internal FrameworkElement GenerateChildrenEditorElement(PropertyItem propertyItem)
        {
            ...
            if (editorElement == null)
            {
                ...
                if (editor == null)
                {
                    editor = (definitionKeyAsType != null) ? PropertyGridUtilities.CreateDefaultEditor(definitionKeyAsType, null, propertyItem) : pd.CreateDefaultEditor(propertyItem);
                }
        ...
        {
            if (propertyItem.Editor != null && ContainerHelper.GetIsGenerated(propertyItem.Editor))
            {
